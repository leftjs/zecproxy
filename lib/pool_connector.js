...
var PoolConnector = function(restartCallback) {
  ...
  this.onError = function(error) {
    ...
    if (error.code == "ECONNRESET") {
      // connection lost, retry on same pool
      this.connect(false);
    } else {
      // Try with next pool
      this.connect(true);
  ...
  this.onData = function(obj) {
    ...
    if (this.poolSocket != null) {
      if (this.status == "subscribing" && obj.id == 1) {
        // Subscription (internal)
        if (obj.error) {
          this.error(obj.error);
        } else {
          // {"id":1,"result":[[["mining.set_difficulty","1"],["mining.notify","688be0542e88112114616a8e7374d3f1"]],"81005fce",4],"error":null}
          this.sessionId = obj.result[1];
          this.subscribeResult = obj.result;
          logger.log("Stratum session id " + this.sessionId);
          var auth = {
            id: 2,
            method: "mining.authorize",
            params: [config.wallet + "." + config.proxy_name, config.password]
          };
          logger.log("Authorizing mining wallet " + config.wallet);
          this.status = "authorizing";
          this.send(auth);
        }
      } else if (this.status == "authorizing" && obj.id == 2) {
  ...
  this.connect = function(try_next_pool) {
    if (this.poollist.length > 0) {
      ...
      if (this.current_pool.ssl) {
        this.poolSocket = tls.connect(
          this.current_pool.port,
          this.current_pool.host,
          () => {
      ...
      this.poolSocket.on("connect", connect => {
      ...
      this.poolSocket
        ...
        .on("error", e => {
          logger.err("invalid pool request, " + e);
        });
      this.poolSocket.on("end", () => {
        this.onEnd();
      });
    } else {
