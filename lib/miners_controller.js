...
var Miner = function(connection, controller) {
  ...
  this.connection
    ...
    .on("error", e => {
      logger.err("invalid worker request, " + e);
    });
  ...
  if (this.connection) {
    ...
    if (obj.method == "mining.subscribe") {
      if (this.ctrl.proxy.sessionId == null) {
        ...
        // this.send({id:this.lastReqId,result:[this.ctrl.proxy.sessionId,this.ctrl.proxy.sessio    nId],error:null});
        this.send({ id: this.lastReqId, result: this.ctrl.proxy.subscribeResult, error: null });
      }
